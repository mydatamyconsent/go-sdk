/*
My Data My Consent - Developer API

Unleashing the power of consent by establishing trust. The Platform Core Developer API defines a set of capabilities that can be used to request, issue, manage and update data, documents and credentials by organizations. The API can be used to request, manage and update Decentralised Identifiers, Financial Data, Health Data issue Documents, Credentials directly or using OpenID Connect flows, and verify Messages signed with DIDs and much more.

API version: 1.0
Contact: support@mydatamyconsent.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package github.com/mydatamyconsent/sdk

import (
	"encoding/json"
)

// FinancialAccountCreditCard struct for FinancialAccountCreditCard
type FinancialAccountCreditCard struct {
	Type string `json:"type"`
	Id string `json:"id"`
	Name string `json:"name"`
	Identifier string `json:"identifier"`
	Balance float64 `json:"balance"`
	Profile CreditCardProfile `json:"profile"`
	Summary CreditCardSummary `json:"summary"`
	MaskedAccountNumber string `json:"masked_account_number"`
	LinkedAccountRef string `json:"linked_account_ref"`
	Version float32 `json:"version"`
}

// NewFinancialAccountCreditCard instantiates a new FinancialAccountCreditCard object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFinancialAccountCreditCard(type_ string, id string, name string, identifier string, balance float64, profile CreditCardProfile, summary CreditCardSummary, maskedAccountNumber string, linkedAccountRef string, version float32) *FinancialAccountCreditCard {
	this := FinancialAccountCreditCard{}
	this.Type = type_
	this.Id = id
	this.Name = name
	this.Identifier = identifier
	this.Balance = balance
	this.Profile = profile
	this.Summary = summary
	this.MaskedAccountNumber = maskedAccountNumber
	this.LinkedAccountRef = linkedAccountRef
	this.Version = version
	return &this
}

// NewFinancialAccountCreditCardWithDefaults instantiates a new FinancialAccountCreditCard object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFinancialAccountCreditCardWithDefaults() *FinancialAccountCreditCard {
	this := FinancialAccountCreditCard{}
	return &this
}

// GetType returns the Type field value
func (o *FinancialAccountCreditCard) GetType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *FinancialAccountCreditCard) GetTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *FinancialAccountCreditCard) SetType(v string) {
	o.Type = v
}

// GetId returns the Id field value
func (o *FinancialAccountCreditCard) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *FinancialAccountCreditCard) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *FinancialAccountCreditCard) SetId(v string) {
	o.Id = v
}

// GetName returns the Name field value
func (o *FinancialAccountCreditCard) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *FinancialAccountCreditCard) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *FinancialAccountCreditCard) SetName(v string) {
	o.Name = v
}

// GetIdentifier returns the Identifier field value
func (o *FinancialAccountCreditCard) GetIdentifier() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Identifier
}

// GetIdentifierOk returns a tuple with the Identifier field value
// and a boolean to check if the value has been set.
func (o *FinancialAccountCreditCard) GetIdentifierOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Identifier, true
}

// SetIdentifier sets field value
func (o *FinancialAccountCreditCard) SetIdentifier(v string) {
	o.Identifier = v
}

// GetBalance returns the Balance field value
func (o *FinancialAccountCreditCard) GetBalance() float64 {
	if o == nil {
		var ret float64
		return ret
	}

	return o.Balance
}

// GetBalanceOk returns a tuple with the Balance field value
// and a boolean to check if the value has been set.
func (o *FinancialAccountCreditCard) GetBalanceOk() (*float64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Balance, true
}

// SetBalance sets field value
func (o *FinancialAccountCreditCard) SetBalance(v float64) {
	o.Balance = v
}

// GetProfile returns the Profile field value
func (o *FinancialAccountCreditCard) GetProfile() CreditCardProfile {
	if o == nil {
		var ret CreditCardProfile
		return ret
	}

	return o.Profile
}

// GetProfileOk returns a tuple with the Profile field value
// and a boolean to check if the value has been set.
func (o *FinancialAccountCreditCard) GetProfileOk() (*CreditCardProfile, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Profile, true
}

// SetProfile sets field value
func (o *FinancialAccountCreditCard) SetProfile(v CreditCardProfile) {
	o.Profile = v
}

// GetSummary returns the Summary field value
func (o *FinancialAccountCreditCard) GetSummary() CreditCardSummary {
	if o == nil {
		var ret CreditCardSummary
		return ret
	}

	return o.Summary
}

// GetSummaryOk returns a tuple with the Summary field value
// and a boolean to check if the value has been set.
func (o *FinancialAccountCreditCard) GetSummaryOk() (*CreditCardSummary, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Summary, true
}

// SetSummary sets field value
func (o *FinancialAccountCreditCard) SetSummary(v CreditCardSummary) {
	o.Summary = v
}

// GetMaskedAccountNumber returns the MaskedAccountNumber field value
func (o *FinancialAccountCreditCard) GetMaskedAccountNumber() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.MaskedAccountNumber
}

// GetMaskedAccountNumberOk returns a tuple with the MaskedAccountNumber field value
// and a boolean to check if the value has been set.
func (o *FinancialAccountCreditCard) GetMaskedAccountNumberOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.MaskedAccountNumber, true
}

// SetMaskedAccountNumber sets field value
func (o *FinancialAccountCreditCard) SetMaskedAccountNumber(v string) {
	o.MaskedAccountNumber = v
}

// GetLinkedAccountRef returns the LinkedAccountRef field value
func (o *FinancialAccountCreditCard) GetLinkedAccountRef() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.LinkedAccountRef
}

// GetLinkedAccountRefOk returns a tuple with the LinkedAccountRef field value
// and a boolean to check if the value has been set.
func (o *FinancialAccountCreditCard) GetLinkedAccountRefOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.LinkedAccountRef, true
}

// SetLinkedAccountRef sets field value
func (o *FinancialAccountCreditCard) SetLinkedAccountRef(v string) {
	o.LinkedAccountRef = v
}

// GetVersion returns the Version field value
func (o *FinancialAccountCreditCard) GetVersion() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.Version
}

// GetVersionOk returns a tuple with the Version field value
// and a boolean to check if the value has been set.
func (o *FinancialAccountCreditCard) GetVersionOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Version, true
}

// SetVersion sets field value
func (o *FinancialAccountCreditCard) SetVersion(v float32) {
	o.Version = v
}

func (o FinancialAccountCreditCard) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["type"] = o.Type
	}
	if true {
		toSerialize["id"] = o.Id
	}
	if true {
		toSerialize["name"] = o.Name
	}
	if true {
		toSerialize["identifier"] = o.Identifier
	}
	if true {
		toSerialize["balance"] = o.Balance
	}
	if true {
		toSerialize["profile"] = o.Profile
	}
	if true {
		toSerialize["summary"] = o.Summary
	}
	if true {
		toSerialize["masked_account_number"] = o.MaskedAccountNumber
	}
	if true {
		toSerialize["linked_account_ref"] = o.LinkedAccountRef
	}
	if true {
		toSerialize["version"] = o.Version
	}
	return json.Marshal(toSerialize)
}

type NullableFinancialAccountCreditCard struct {
	value *FinancialAccountCreditCard
	isSet bool
}

func (v NullableFinancialAccountCreditCard) Get() *FinancialAccountCreditCard {
	return v.value
}

func (v *NullableFinancialAccountCreditCard) Set(val *FinancialAccountCreditCard) {
	v.value = val
	v.isSet = true
}

func (v NullableFinancialAccountCreditCard) IsSet() bool {
	return v.isSet
}

func (v *NullableFinancialAccountCreditCard) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFinancialAccountCreditCard(val *FinancialAccountCreditCard) *NullableFinancialAccountCreditCard {
	return &NullableFinancialAccountCreditCard{value: val, isSet: true}
}

func (v NullableFinancialAccountCreditCard) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFinancialAccountCreditCard) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


